...
def run_emulation(fault_index):
    ...
    FAULT_ADDR = ADDRESS + fault_index * 4  
    code_arr[FAULT_ADDR:FAULT_ADDR + 4] = NOP
    ...
    mu.emu_start(ADDRESS, ADDRESS + len(CODE), count=100) #stops after 100 instructions
...
baseline = run_emulation(-1)  # no modification
print(f"Baseline R0 = {baseline}")

for i in range(len(CODE)): run_emulation(i)
